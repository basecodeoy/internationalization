...
it('creates from valid currency code string', function (): void {
    ...
    $currencyCode = CurrencyCode::createFromString($validCurrencyCode);

    expect($currencyCode->toString())->toEqual($validCurrencyCode);
});
...
it('returns correct localized string representation', function (): void {
    ...
    expect($currencyCode->localized)->toEqual('US Dollar');
});
